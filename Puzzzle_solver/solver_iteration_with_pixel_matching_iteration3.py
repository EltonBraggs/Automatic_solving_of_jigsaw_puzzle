...
def main():
    ...
    for j in range(len(pixel_matching_list)):
        ...
        for i in range(0,len(pixel_matching_list)-1):
            W = [np.array(0.2 * np.array(pixel_matching_list[i][1][0])) , np.array(0.2 * np.array(pixel_matching_list[i][1][1])) , np.array(0.2 * np.array(pixel_matching_list[i][1][2])) , np.array(0.2 * np.array(pixel_matching_list[i][1][3]))]
                ...
                for k in range(len(A)):
                    for m in range(len(D)):
                        ...
                        #print('E[m][2]' , E[m][2])
                            ...
                            matching_pixels.update({j:{pixel_matching_list[j][0]: A[k] , 'matching_side1' : list_name1 , pixel_matching_list[i][0] : pixel_matching_list[i][1][m] , 'matching_side' : list_name}})
